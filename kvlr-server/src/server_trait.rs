...
pub trait SomeFunctions: Send + Sync + 'static {
    ...
    async fn range_vec(&self, connection: Arc<Connection>, arg0: u32) -> Vec<u32>;
    async fn call_me_to_panic(&self, connection: Arc<Connection>, ) -> ();
    ...
    fn register(this: Arc<Self>, fns_map: &mut HashMap<u32, Arc<dyn HandlerFn>>) {
        ...
        {
            let this = this.clone();
            fns_map.insert(4321, into_handler(move |conn, pld, slice: Vec<u8>| {
                let this = this.clone();
                async move {
                    ...
                    let out = this.range_vec(conn, args.0,).shared().await;
                    rmp_serde::to_vec(&out).unwrap()
                }
            }));
        }
        ...
        {
            let this = this.clone();
            fns_map.insert(1111, into_handler(move |conn, pld, slice: Vec<u8>| {
                let this = this.clone();
                async move {
                    

                    let out = this.call_me_to_panic(conn, ).shared().await;
                    rmp_serde::to_vec(&out).unwrap()
                }
            }));
        }
